...
namespace TestProtoGen {
    public class TicketData {
        ...
        public TicketData(byte[] TicketIn) {
            Ticket = TicketIn;
            Unpack() ;
            }
        ...
        static void DumpCrypto (string tag, byte[] Ticket, Cryptography.Key SeedKey) {
            Console.WriteLine ("Dump Crypto {0} Seed={1} Ticket={2}",
                tag, Convert.ToBase64String(SeedKey.KeyData), Convert.ToBase64String(Ticket));
            }


        static byte[] Pack(byte[] Ticket, Cryptography.Key SeedKey) {
            byte [] Result = null;
            byte [] Hash = null;
        ...
        static byte[] UnPack(byte[] Ticket, Cryptography.Key SeedKey) {
            byte[] Decrypted = null;
            int IVlength;
            using (AesManaged Aes = new AesManaged()) {
                Aes.Key = SeedKey.KeyData;
        ...
        public static TicketData MakeTicket(byte[] WrappedTicket, Cryptography.Key SeedKey) {
            ...
            byte[] Ticket = UnPack (WrappedTicket, SeedKey);
            ...
            if (x != 0) throw new Exception ("Bad ticket");
            x = Ticket [index++];
            if (x == 0) return new TicketData (Ticket);
            if (x == 1) return new TemporaryTicketData (Ticket);
    ...
    public class TemporaryTicketData : TicketData {
        ...
        public TemporaryTicketData(byte[] TicketIn) {
            Ticket = TicketIn;
            int index = Unpack() ;
            int x;
